...
class InventoryManagerTestCase(TestCase):
    ...
    def test_sr_product_relateds(self):
        # We just need to verify that calling the manager method returns a queryset
        # and doesn't crash. Detailed SQL verification is usually not needed for unit tests
        # unless checking optimization specifically, but we can check if it joins.
        qs = Inventory.objects.sr_product_relateds()
        self.assertIn(self.inventory, qs)
        # Verify it executes without error and returns results
